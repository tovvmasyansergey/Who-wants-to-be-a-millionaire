...
def ask_questions(name, questions, num_questions=10):
    ...
    Brief: Presents questions to the user in a window and tracks their score.
    Params: name - the user's name
            questions - a list of dictionaries with questions and answers
    ...
    def next_question(index=0):
        nonlocal correct_answer,phone_number,answers_1,answers
        if index < len(sample_questions):
            ...
            answers_1 = []
            for ans in answers:
                answers_1.append(ans)
            random.shuffle(answers)
            for i,answer in enumerate(answers):
                answer_buttons[i].config(text=answer,state = tkinter.NORMAL, command=lambda ans=answer[0]: check_answer(ans, question["ans"].split(',')[0], index))
    ...
    def use_50(corr_ans,answers):
        nonlocal a_5050
        if a_5050:
            a_5050 = False
            incorrect_answer = [ans for ans in answers if ans.strip().lower() != corr_ans.strip().lower()]
            for btn in answer_buttons:
                if btn['text'] in incorrect_answer[:2]:
    ...
    def use_call():
        ...
        if phone_number:
            phone_number = False
            messagebox.showinfo("Phone of friend")
    def use_correct(right_answer):
        nonlocal correct_answer
        if correct_answer:
            correct_answer = False
        messagebox.showinfo("correct answer",f'{right_answer}')
    ...
    for btn in answer_buttons:
        btn.pack(fill='x')
    used_5050 = tkinter.Button(question_window,text = "50/50",command = lambda : use_50(answers_1[0],answers))
    used_5050.pack()
...
def get_name_and_start_quiz():
    """
    Function: get_name_and_start_quiz
    Brief: Gets the user's name from the input and starts the quiz.
    Params: None
    Return: None
    """
    name = name_entry.get()
